...
Every `triangleIndex`/`triangleIndices` field this package reports anywhere -- region evidence, topology-finding examples, interference `trianglePairs`, printability island components, and so on -- is an index into `flattenModel`'s flattened comparison-frame geometry (`src/geometry.ts`) for the specific `(model, modelToComparison)` pair that produced it, never a raw mesh vertex/triangle index and never stable across a different placement transform.

**Traversal order is a documented, tested stability guarantee, not an implementation detail.** `flattenModel` visits mesh instances in exactly this order: `placement.kind === "flat"` walks `placement.instances` in array order; `placement.kind === "hierarchy"` walks `placement.rootIds` in array order, then a pre-order depth-first walk of each root's node subtree -- each node's own `instanceIds` (in array order) are queued before descending into its `childIds` (in array order, each child's entire subtree queued before moving to the next child). Within each queued instance, vertices and triangles are appended in that instance's own mesh's `geometry.positions`/`geometry.indices` order. Consequently, each instance contributes one contiguous, non-overlapping run of flattened triangle indices, in that order, whose length equals its own mesh's triangle count -- see `flattenModel`'s doc comment for the full statement. Changing this order is a breaking change to this package's contract, exactly like changing a method's result shape, and every part of it is pinned behaviourally, not just asserted in prose, by `test/flatten-order.test.ts`: fixtures place every triangle at its own distinct, hand-computed world-space position -- across multiple meshes, multiple instances of the same mesh, and a hierarchy placement with nested transforms -- so a specific flattened index only matches its expected position if mesh iteration, instance expansion, and the placement-tree walk are exactly as documented; a future reordering of any of them fails those tests loudly, not silently.

**Resolve reported indices with `flattenedTriangleLocator`/`resolveFlattenedTriangle` (`src/triangle-locator.ts`) -- never by re-deriving `flattenModel`'s traversal in consumer code.** `flattenedTriangleLocator(model, options?)` flattens `model` once (reusing `flattenModel` exactly as every other entry point in this package does, under the same `ANALYSIS_LIMITS` ceilings and charge-before-work `WorkBudget` discipline) and returns a `{ geometry, triangleCount, resolve }` object whose `resolve(triangleIndex)` maps one flattened index to its provenance and world-space geometry: `{ triangleIndex, meshId, instanceId, meshLocalTriangleIndex, positionsMillimetres }`, where `meshLocalTriangleIndex` is that triangle's index within its own mesh's `geometry.indices`, independent of placement. Build one locator per `(model, modelToComparison)` and call `resolve` for every index a caller needs (e.g. every entry in one region's `triangleIndices`) -- the locator's own O(vertices + triangles) flatten happens once at construction, and each `resolve` call after that is O(log instances), a binary search over precomputed per-instance triangle-range boundaries, never a re-flatten. `resolveFlattenedTriangle(model, triangleIndex, options?)` is a one-shot convenience wrapper over the same locator, for a caller that only needs a single index.

The `modelToComparison` passed to the locator must be the exact same rigid transform the index being resolved was originally reported under -- resolution has no way to detect a mismatched transform, since any rigid transform flattens to the same triangle count and order, only different coordinates. `flattenedTriangleLocator` fails closed exactly like `measureOnModel`: `model` is validated against `normalizedModelSchema`, `modelToComparison` (default identity) is validated as a proper rigid transform, expanded vertex/triangle counts and estimated memory are checked before any O(vertices + triangles) work runs (throwing `TriangleLocatorResourceLimitError`), and flattening runs under a charge-before-work `WorkBudget` (throwing `WorkBudgetExceeded`/`WorkBudgetInternalError` unchanged on exhaustion or misuse). `resolve` itself rejects any `triangleIndex` outside `[0, triangleCount)` -- including non-finite or non-integer input -- with `TriangleLocatorInputError` rather than reading out of bounds. Both are deterministic: identical `(model, modelToComparison)` produces an identical `geometry` and identical `resolve` results every time, verified against a directly-flattened reference geometry in `test/triangle-locator.test.ts`.

**The one-line guidance for a UI author:** to turn a reported `triangleIndex` into a triangle to highlight, call `flattenedTriangleLocator(model, { modelToComparison }).resolve(triangleIndex)` -- do not reconstruct `flattenModel`'s mesh/instance/placement-tree walk yourself.
...
**This is not a printability verdict, and it never becomes one.** `assessPrintability(model, options?)` reports bounded, deterministic evidence and warnings about a single model's tessellated surface -- thin regions, overhangs, disconnected shells, build-volume fit, and dimensional plausibility -- for a human (or a slicer) to weigh. It never concludes that a model "will print" or "will not print": that conclusion depends on slicer settings, material, and printer calibration this package has no access to and no domain-validated model of. Every `PrintabilityAssessment` carries `disclaimer`, verbatim equal to the exported `PRINTABILITY_DISCLAIMER` constant, so a UI never has to invent its own version of this caveat. The five checks below are never aggregated into one pass/fail outcome -- each measures a different thing with a different kind of precision, reports its own `semantics`, and must be shown to a user as what it actually is, not folded into a single score.

**`wallThickness`** (`semantics: "approximate-directional-probe"`). For a bounded, deterministically selected set of triangles (a fixed stride walk over triangle order, `maxSampleTriangles`, default `DEFAULT_MAX_WALL_THICKNESS_SAMPLE_TRIANGLES` = 20,000, ceiling `MAX_WALL_THICKNESS_SAMPLE_TRIANGLES` = 200,000), a ray is cast from each sampled triangle's centroid straight in along that triangle's own inverted (inward-pointing) normal, and the distance to the first opposite-surface hit is reported as `thicknessMillimetres` wherever it falls below `thinThresholdMillimetres` (default `DEFAULT_THIN_WALL_THRESHOLD_MILLIMETRES` = 0.8mm -- an adjustable, arbitrary default, not a printer- or material-specific recommendation). **This is directional, not the true local minimal thickness**: a smaller thickness can exist along a different direction (e.g. diagonally through a corner) than the one probed. It is also silent about any triangle never sampled at all (`unsampledTriangleCount`, when the mesh exceeds `maxSampleTriangles`) and about any probe whose ray happened to find no opposite surface (`missedProbeCount` -- an open mesh boundary, a degenerate source triangle, or inconsistent winding can each cause this; these contribute no finding rather than a silently wrong one). Coverage is bounded, not just disclosed in prose: `sampleSpacingUpperBoundMillimetres` is an upper bound, in millimetres, on the distance from any point on a _sampled_ triangle to that triangle's own probed centroid -- at most two-thirds of that triangle's longest edge, the same convex-polygon argument behind `surface-distance`'s `SAMPLE_SPACING_EDGE_FACTOR` bound (the farthest point in a triangle from an interior point is always a vertex, and a centroid is at most two-thirds of the longest edge from any vertex by the triangle inequality), specialized here to a single centroid sample rather than vertices-plus-centroid. This bound says nothing about `unsampledTriangleCount` triangles, which have no coverage guarantee at all -- exactly why a coarse mesh's large reported bound, or a nonzero `unsampledTriangleCount`, is how this check acknowledges that a thin feature between samples could be missed entirely, not a silent gap. `findings` is ascending by `thicknessMillimetres` (thinnest first), bounded by `maxFindings` (default `DEFAULT_MAX_WALL_THICKNESS_FINDINGS` = 50, ceiling `MAX_WALL_THICKNESS_FINDINGS` = `ANALYSIS_LIMITS.maxReportedRegions` = 2,048), with the true `findingCount` and a `truncated` flag reported independently.

The ray cast itself is a genuine Moller-Trumbore intersection test (`rayTriangleIntersectionT` in `src/geometry.ts`), accelerated by a new BVH ray traversal, `TriangleSpatialIndex.castRayNearest` (`src/spatial-index.ts`), added specifically to make probing many triangles at scale tractable -- `src/measure.ts`'s existing `castRay` remains an unmodified, independent full linear scan for click-to-measure's single-ray use, and `TriangleSpatialIndex.distance`/`nearestTriangle`/`overlapping` are unchanged. `castRayNearest` traverses the same deterministic AABB tree `nearestTriangle` does, pruning by each node's ray-entry distance (a standard slab test, with each zero-direction-component axis handled explicitly rather than relying on IEEE-754 arithmetic with an infinite reciprocal, which produces `NaN` when a ray origin lands exactly on a slab boundary) instead of point-to-box distance. `test/spatial-index-ray-property.test.ts` validates it against an independent brute-force linear scan built from the same shared `rayTriangleIntersectionT` primitive, across random and adversarial (sliver, collinear, axis-aligned, boundary-touching) triangle clouds -- the same discipline `test/spatial-index-property.test.ts` already applies to `distance`/`nearestTriangle`.

**`overhangs`** (`semantics: "exact-for-tessellated-surface"`). For every triangle -- not a sample -- the exact angle between its own outward normal and `buildDirection` (default `[0, 0, 1]`, normalized internally) is computed as degrees from vertical: `0` is a vertical wall, `90` is a flat, directly downward-facing ceiling. A triangle is flagged when that angle is _strictly greater_ than `thresholdDegreesFromVertical` (default `DEFAULT_OVERHANG_THRESHOLD_DEGREES_FROM_VERTICAL` = 45, must be between 0 and 90). This is exact for the tessellated mesh -- ordinary floating-point arithmetic on exact input coordinates, ordinary rounding aside, not a sampled approximation -- in contrast to `wallThickness` above, and reports `overhangAreaSquareMillimetres`, `totalSurfaceAreaSquareMillimetres`, and their `overhangAreaFraction`, all exact sums over triangle areas. Flagged triangles are grouped into `regions` using the same exact-coordinate edge connectivity (`groupTrianglesByExactEdgeConnectivity`, `src/region-connectivity.ts`) `surface-distance` and `checkClearance` already use -- never a second connectivity implementation -- ranked descending by area, bounded by `maxRegions` (default `DEFAULT_MAX_OVERHANG_REGIONS` = 50, ceiling `MAX_OVERHANG_REGIONS` = `ANALYSIS_LIMITS.maxReportedRegions`), with `detectedRegionCount` and `truncated` reported independently of the bound.

**`islands`** (`semantics: "exact-connectivity"`). Connected components (triangle shells joined by shared exact-coordinate edges) are counted with the same shared implementation `summarizeModelGeometry`'s own `componentCount` and `surface-distance`'s region grouping already use -- reused, per this check's own requirement, rather than reimplemented. Each reported component (bounded by `maxComponents`, default `DEFAULT_MAX_ISLAND_COMPONENTS` = 200, ceiling `MAX_ISLAND_COMPONENTS` = `ANALYSIS_LIMITS.maxReportedRegions`, descending by `triangleCount`) carries its `triangleCount`, its axis-aligned `bounds`, and a `volume` that is `{ available: true, cubicMillimetres }` only when that _component itself_ -- not the whole model -- is closed, consistently oriented, and free of degenerate triangles (checked with a component-scoped edge census and the same signed-tetrahedron volume formula `summarizeModelGeometry`'s own volume computation uses, both newly scoped to one component's triangle subset since neither existing implementation operates over an arbitrary subset), else `{ available: false, reasons }` naming which of those conditions failed. `componentCount > 1` always adds a `printability.multiple-islands` warning -- a stray, unintended fragment prints as loose material, not part of the intended object, and this check cannot tell intentional multi-shell models (e.g. a two-part assembly) from that apart, so it surfaces the fact and leaves the judgment to the caller.

**`buildVolume`**. `semantics: "not-configured"` (only `dimensionsMillimetres`, the model's own axis-aligned extents, are reported) unless `options.buildVolume.dimensionsMillimetres` is supplied, in which case `semantics: "exact-axis-aligned-fit"` and the model's extents are checked against all six axis-order permutations -- **only axis permutations, never an arbitrary rotation search**, exactly as this check's documented scope requires. `orientations[0]` is always the as-given `[0, 1, 2]` orientation (`fitsAsGiven` mirrors its `fits`); each of the six reports `exceedsByMillimetres` per build axis (`0` where that axis fits, else the positive millimetre excess), so both "does it fit" and "by how much does it miss" are always available together, for every orientation, not just the best one. `fitsInAnyOrientation` is `true` when any of the six fits; a `printability.exceeds-build-volume` warning fires when none does, and a `printability.fits-only-when-reoriented` warning fires when only a non-identity orientation fits.

**`scale`**. Reports the model's own `dimensionsMillimetres` (in the canonical comparison frame) alongside `sourceUnit`/`detectedSourceUnit`, echoed unmodified from `model.provenance` -- this package never guesses, re-derives, or corrects units. `implausible` is set, with `implausibleReason` naming `"smaller-than-typical-print-scale"` or `"larger-than-typical-build-volume"`, when the largest dimension falls outside `MIN_PLAUSIBLE_MODEL_DIMENSION_MILLIMETRES` (1mm) to `MAX_PLAUSIBLE_MODEL_DIMENSION_MILLIMETRES` (1,000mm) -- a range typical of common desktop printers, surfaced as **an observation about the resolved millimetre dimensions, never a claim about what the original file's units actually were or a correction applied to them**.

**Aggregation.** `warnings` is the union of every check's own warnings (wall thickness, then overhangs, then islands, then build volume, then scale), each carrying a stable `code`, a `severity` (`"info"` for bounded-report truncation notices, `"warning"` for evidence a caller should probably act on or verify), a human-readable `message`, and optional numeric `details`. `method.id`/`method.version` (`PRINTABILITY_METHOD_ID` = `"printability-inspector"`, `PRINTABILITY_METHOD_VERSION` = `"1.0.0"`) are reported for provenance, the same as `checkClearance`'s `method`.

**Resource discipline.** `model` is validated against `normalizedModelSchema`, then expanded vertex/triangle counts (plus estimated memory, honoring an optional caller-supplied `executionBudget.maxMemoryBytes`) are checked via `checkExpandedGeometryBudget` -- the same pre-flight `checkClearance`, `estimateAlignment`, and `measureOnModel` use -- throwing `PrintabilityResourceLimitError` before any O(vertices + triangles) work runs. Flattening, spatial-index construction, wall-thickness ray casts, overhang classification, and island connectivity are all charged to one charge-before-work `WorkBudget` (bounded by `executionBudget.maxWorkUnits`, reusing `ANALYSIS_LIMITS`/`WorkBudget` unchanged); an exhausted budget throws `WorkBudgetExceeded` unchanged, matching every other entry point in this package. `options.modelToComparison` is validated as a proper rigid transform (no scale, shear, or reflection -- distorting placed geometry would distort every thickness, angle, and extent this function reports). Invalid option values (an out-of-range bound, a non-finite or degenerate `overhang.buildDirection`, a non-finite or non-positive `buildVolume.dimensionsMillimetres`) throw `RangeError`/`PrintabilityInputError` respectively, matching this package's other entry points' conventions.

**Determinism.** Identical input produces a deeply equal `PrintabilityAssessment` every time: sample-triangle selection is a fixed stride walk over triangle order, `TriangleSpatialIndex` traversal is deterministic, region/component grouping and ranking use full deterministic tie-breaking, and no step introduces randomness.

## Certified mesh simplification

Mesh decimation by itself is a commodity: many tools reduce a triangle count. `simplifyModel(model, options)` decimates a mesh, then **measures** the actual, sampled deviation between the original and simplified geometry using this package's own comparison machinery and reports a bound -- "37% fewer triangles, maximum measured deviation 0.03 mm" is a claim this function can back with evidence, not just assert. The certification, not the decimation, is the point.

**Decimation** (`method.id` = `SIMPLIFY_METHOD_ID` = `"quadric-edge-collapse"`, version `SIMPLIFY_METHOD_VERSION` = `"1.0.0"`) is a self-contained, dependency-free implementation of Garland-Heckbert quadric-error-metric edge collapse:

1. `model` is flattened into one comparison-frame geometry in its own frame (`flattenModel` with an identity transform), then vertices are welded by exact coordinate identity -- the same no-tolerance convention every other connectivity computation in this package uses (see "Topology semantics" above) -- so a facet-local mesh (private vertex copies per triangle corner, as binary STL import commonly produces) is recognized as having shared edges wherever its duplicated corners coincide exactly, the same way `assessGeometry`/`groupTrianglesByExactEdgeConnectivity` already do.
2. Each welded vertex accumulates a quadric error matrix from the planes of its incident triangles. Each interior edge (shared by exactly two triangles) is a collapse candidate, scored by the quadric error of its numerically optimal merge point (a 3x3 linear solve, falling back to the least-bad of its two endpoints or their midpoint when that system is singular -- e.g. perfectly flat local geometry).
3. Candidates are processed from a binary min-heap in a strict total order -- ascending error, then ascending canonical vertex-id pair, then insertion sequence -- with lazy staleness detection (an edge whose endpoint moved since it was scored is discarded, not reused), until the requested triangle count is reached or no valid collapse remains. No randomness, no dependence on `Map`/`Set` iteration beyond the language's own guaranteed insertion order.

Every candidate is validated before it is applied, and a candidate that fails validation is skipped -- never forced through and never retried unchanged:

- **Boundary preservation.** By default (`collapseBoundaryEdges: false`), no vertex that touches any boundary edge (shared by exactly one triangle) is ever a collapse candidate, in either role. This is stronger than "boundary edges are never collapsed": a boundary _vertex_ also cannot be moved by collapsing some other, interior edge touching it. An open surface's boundary loops -- their count, their edges, their exact positions -- are therefore untouched by default, not merely "not directly collapsed." Setting `collapseBoundaryEdges: true` lifts this and adds a `simplify.boundary-edges-collapsible` warning whenever the input actually has boundary edges.
- **Manifoldness.** An edge already shared by more than two triangles is never a candidate. Every remaining candidate is checked against the standard link condition (the intersection of its two endpoints' neighbor-vertex sets must equal exactly the opposite vertices of the triangles the collapse would remove) before being applied. The link condition alone is insufficient for a very small closed component -- collapsing any edge of a tetrahedron (exactly 4 triangles) satisfies it while merging the two remaining triangles into an identical, zero-thickness "doubled" face -- so every candidate is additionally checked against a general duplicate-triangle guard that rejects a collapse if it would leave any two surviving triangles sharing the exact same three vertices. Together these keep a closed component from ever being silently decimated below its true manifold floor (4 triangles for a closed surface).
- **Orientation safety.** A collapse that would flip a surviving triangle's normal by 90 degrees or more (or collapse it to zero area) is skipped.

If validation empties the heap before the requested target is reached, decimation simply stops there: `reduction.targetReached` is `false`, the actually-achieved triangle count is reported honestly on `simplified.triangleCount`, and a `simplify.target-not-reached` warning explains why -- this never fails and never fabricates unreachable geometry.

**Certification** (`certification.method.id` = `CERTIFICATION_METHOD_ID` = `"surface-distance-both-directions"`) measures both directions -- original-to-simplified and simplified-to-original, since either alone can understate the difference -- reusing exactly the vertices-plus-centroid sampling `analyzeModelPair`'s `surface-distance` method uses, against a `TriangleSpatialIndex` of the opposite geometry. `certification.semantics` is always `"approximate-sampled-bound"`; it is **never presented as an exact or guaranteed Hausdorff distance**. `certification.disclaimer` (always `CERTIFICATION_DISCLAIMER`, verbatim) states this precisely: the reported maximum is the true maximum among the samples actually taken, qualified by `sampleSpacingUpperBoundMillimetres` (per direction and combined) -- the same "at most two-thirds of the coarsest analyzed triangle's longest edge" bound `surface-distance` reports, below which a genuine deviation confined to one triangle's interior could be missed entirely. Each direction also reports its own `worstSampleMillimetres`/`worstSampleSourceTriangleIndex`, so a UI can highlight exactly where the certified maximum was measured, not just its value.

**Result shape.** `SimplificationResult` reports `original`/`simplified` triangle and vertex counts, `reduction` (triangles removed, the achieved ratio, and `targetReached`), `parameters` (the requested target echoed back alongside the resolved absolute `effectiveTargetTriangleCount` and `collapseBoundaryEdges`), the `certification` above, a bounded set of `warnings` (flattened multi-mesh/multi-instance placement, excluded degenerate input triangles, opted-in boundary collapse, target-not-reached -- each disclosed once, here, never silently), and `model`: the simplified geometry as a `NormalizedModel`, already validated against `normalizedModelSchema` before it is returned. That model is a normal citizen of this package -- it can be fed straight into `inspectModel`, `assessPrintability`, `diagnoseMeshHealth`, or `analyzeModelPair` like any other.

**Resource discipline.** `model` is validated against `normalizedModelSchema`, then checked against both the shared `checkExpandedGeometryBudget` every multi-triangle entry point in this package uses and a separate, tighter `MAX_SIMPLIFY_INPUT_TRIANGLES` (200,000) ceiling -- documented, not benchmarked, because edge-collapse decimation's working set (per-vertex quadrics, adjacency sets, a priority heap) costs materially more per triangle than the flatten-and-sample passes `ANALYSIS_LIMITS` was calibrated against. Welding, quadric initialization, candidate scoring, heap operations, collapse validation and application, and both certification passes are all charged to one charge-before-work `WorkBudget` (bounded by `executionBudget.maxWorkUnits`, reusing `ANALYSIS_LIMITS`/`WorkBudget` unchanged); an exhausted budget throws `WorkBudgetExceeded` unchanged. Invalid `target` values (a triangle count not strictly below the input's own count, or a reduction ratio outside `(0, 1)`) throw `SimplifyInputError`; a request the resource ceilings cannot accommodate throws `SimplifyResourceLimitError` -- both before any O(vertices + triangles) work runs.

**Determinism.** Identical input produces a deeply equal `SimplificationResult` every time, including the emitted `model`'s own geometry (vertex positions and triangle indices): welding, quadric accumulation, candidate ordering, the binary heap's pop sequence, and final vertex/triangle renumbering are all fixed functions of the input, with no randomness anywhere in the pipeline.
...
const result = simplifyModel(model, {
  target: { kind: "reduction-ratio", reductionRatio: 0.5 },
});
console.log(
  `${(result.reduction.triangleReductionRatio * 100).toFixed(0)}% fewer triangles, ` +
    `maximum measured deviation ${result.certification.maximumDistanceMillimetres.toFixed(3)} mm`,
);
if (!result.reduction.targetReached) {
  console.log("Target not fully reached:", result.warnings);
}
// result.model is a schema-valid NormalizedModel, ready for inspectModel/assessPrintability/analyzeModelPair.
```
...
```ts
import { assessPrintability } from "@voxelspy/analysis";

// Evidence and warnings, never a printability verdict -- see `report.disclaimer`.
const report = assessPrintability(model, {
  buildVolume: { dimensionsMillimetres: [220, 220, 250] },
});
console.log(report.disclaimer);
for (const finding of report.wallThickness.findings) {
  console.log(finding.thicknessMillimetres, finding.positionMillimetres);
}
for (const region of report.overhangs.regions) {
  console.log(region.maxAngleFromVerticalDegrees, region.areaSquareMillimetres);
}
if (
  report.buildVolume.semantics === "exact-axis-aligned-fit" &&
  !report.buildVolume.fitsAsGiven
) {
  console.warn("Does not fit as placed:", report.buildVolume.orientations);
}
