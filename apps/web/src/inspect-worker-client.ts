 ...
 * heavier opt-in mesh-health evidence (`"diagnose"`); both re-run the same
 * local import from the same source bytes, since a fresh dedicated worker is
 * spun up and torn down per call (see `runInspectWorker` below) rather than
 * keeping an imported model alive across requests -- the file is small
 * (bounded by the shared 32 MiB / 500,000-triangle importer ceiling both
 * calls apply), so re-importing costs far less than the complexity of a
 * stateful worker just to avoid it.
 */
