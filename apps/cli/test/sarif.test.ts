...
describe("buildSarifLog", () => {
  ...
  it("emits exactly one result per finding, with the finding's ruleId and message", () => {
    ...
    const run = (log["runs"] as Record<string, unknown>[])[0] as Record<
      string,
      unknown
    >;
  ...
  it("maps a policy-failure rule to level error", () => {
    const log = buildSarifLog(
      baseInput([
        ...
        {
          ruleId: "region-count-exceeds-threshold",
          message: "x",
          artifactUris: ["a.stl"],
        },
      ...
      baseInput([
        {
          ...
          message: "sampled, not exact",
          artifactUris: ["a.stl"],
        },
        {
          ruleId: "undersampled-region",
          message: "spacing exceeds tolerance",
          artifactUris: ["a.stl"],
        },
      ]),
    );
  ...
  it("never invents a source region -- every location is an artifact URI only", () => {
    ...
    for (const location of locations) {
      const physicalLocation = location["physicalLocation"] as Record<
        string,
        unknown
      >;
  ...
  it("carries method/tolerance/sampling-bound/uncertainty provenance in run properties", () => {
    ...
    expect(properties["method"]).toEqual({
      id: "surface-distance",
      version: "1.0.0",
    });
